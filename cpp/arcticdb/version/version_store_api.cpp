...
VersionResultVector PythonVersionStore::list_versions(
    ...
    const bool do_iterate = opt_false(iterate_on_failure);
...
ReadResult PythonVersionStore::read_dataframe_version(
    ...
    const ReadOptions& read_options) {
    auto [versioned_item, frame_and_descriptor] =  read_dataframe_version_internal(stream_id, version_query, read_query, read_options).get();
...
void PythonVersionStore::delete_all_versions(const StreamId& stream_id) {
    ...
    if (!has_stream(stream_id, std::nullopt, std::nullopt)) {
