...
std::string type_to_operator_string(ColumnStatTypeInternal type) {
    const std::unordered_map<ColumnStatTypeInternal, std::string> map {
        {ColumnStatTypeInternal::MIN, "MIN"},
        {ColumnStatTypeInternal::MAX, "MAX"}
    };
    auto it = map.find(type);
    internal::check<ErrorCode::E_ASSERTION_FAILURE>(it != map.end(), "Unknown column stat type requested");
    return it->second;
}
...
std::pair<std::string, ColumnStatType> from_segment_column_name_v1(std::string_view pattern) {
    ...
    const std::unordered_map<ColumnStatTypeInternal, ColumnStatType> internal_to_external_column_stat_type {
        {ColumnStatTypeInternal::MIN, ColumnStatType::MINMAX},
        {ColumnStatTypeInternal::MAX, ColumnStatType::MINMAX}
    };
    return std::make_pair(std::string(pattern.substr(1, pattern.size() - 2)), internal_to_external_column_stat_type.at(*type));
}

std::string type_to_name(ColumnStatType type) {
    const std::unordered_map<ColumnStatType, std::string> map {
        {ColumnStatType::MINMAX, "MINMAX"}
    };
    auto it = map.find(type);
    internal::check<ErrorCode::E_ASSERTION_FAILURE>(it != map.end(), "Unknown column stat type requested");
    return it->second;
...
std::optional<ColumnStatType> name_to_type(const std::string& name) {
    const robin_hood::unordered_flat_map<std::string, ColumnStatType> map {
        {"MINMAX", ColumnStatType::MINMAX}
    };
...
robin_hood::unordered_flat_set<std::string> ColumnStats::segment_column_names() const {
    ...
    const std::unordered_map<ColumnStatType, std::unordered_set<ColumnStatTypeInternal>> external_to_internal_column_stat_type {
        {ColumnStatType::MINMAX, {ColumnStatTypeInternal::MIN, ColumnStatTypeInternal::MAX}}
    };
    robin_hood::unordered_flat_set<std::string> res;
    for (const auto& [column, column_stat_types]: column_stats_) {
        for (const auto& column_stat_type: column_stat_types) {
            for (const auto& column_stat_type_internal: external_to_internal_column_stat_type.at(column_stat_type)) {
